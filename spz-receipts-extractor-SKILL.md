...
description: >
  Extracts individual receipt images from a multi-page PDF, reads each one
  with AI vision, and renames them to a structured filename that captures
  date, vendor, category, amount, and whether it is a sale or refund.

  Output:
  1. Individual PNG files named:
       Sale:   YYYYMMdd_VENDOR_CATEGORY_$X.XX.png
       Refund: YYYYMMdd_VENDOR_CATEGORY_RET_$X.XX.png
  2. A receipts.csv file summarising all receipts with columns:
       date, filename, vendor, category, amount, items, refund, payment_method

  Accepts both multi-page PDFs and individual image files (PNG, JPG, JPEG,
  TIFF, WEBP). Images that cannot be identified as receipts are skipped and
  logged as errors rather than renamed.

  Trigger this skill whenever the user mentions receipts, expense images,
  scanning receipts, organizing receipts, or asks to rename / sort / extract
  receipt files from a PDF or folder — even if they don't use the exact words
  "receipt processor". Also trigger when they upload or point to a PDF or
  folder of images that look like they could be receipts.
---
...
| `vendor` | Store name, UPPERCASE, no spaces. e.g.